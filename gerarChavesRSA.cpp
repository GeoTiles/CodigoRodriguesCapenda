...
void chaves(mpz_t p ,mpz_t q){
  ...
  gmp_printf("\n Totiente(n) = %Zd\n",fi);
  //std::cout << "\n A chave pública é (" << d << "," << n << ")" << std::endl;
  gmp_printf("\n A chave pública é (%Ff,%Zd)\n",d,n);
  //std::cout << "\n A chave privada é (" << e << "," << n << ")" << std::endl;
  gmp_printf("\n A chave privada é (%Zd,%Zd)\n",e,n);
}
...
int main(int argc, char *argv[]){
  ...
  if (argc < 3) {
    cout << "Número insuficiente de argumentos\n\n" << "Utilização: ./gerarChavesRSA número_primo1 número_primo2\n";
    return 1;
  }
  ...
  if(!testaPrimo(p) || !testaPrimo(q)){
    cout << "Valor de p ou q inválidos" << endl << "Entre com  números primos\n";
    return 2;
  }
  else {
    cout << "Valores de p e q válidos\n";
  }
  // gerar as chaves;
  return 0;
}
